...
const MAX_LEN: u32 = (1 << 31) - 1;
const CR32: Crc<u32> = Crc::<u32>::new(&crc::CRC_32_ISO_HDLC);
...
impl Chunk {
    pub fn new(chunk_type: ChunkType, chunk_data: Vec<u8>) -> Self {
        let arr = &[&chunk_type.bytes(), chunk_data.as_slice()].concat();
        let crc = CR32.checksum(arr);
    ...
    pub fn data_as_string(&self) -> Result<String> {
        match String::from_utf8(self.data.clone()) {
            Ok(res) => Ok(res),
            Err(_) => Err("Invalid utf8 data".into()),
        }
    }
...
impl TryFrom<&[u8]> for Chunk {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        ...
        let mut buffer = Vec::from(&bytes[8..bytes.len() - 4]);
        reader.read_exact(&mut buffer)?;

        let cnk = Chunk::new(chunk_type, buffer);
