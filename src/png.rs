...
impl fmt::Display for PngError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PngError::TooSmall => write!(f, "Insufficient Png Data"),
...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let removed = self.chunks.remove(chunk_idx);
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        if bytes.len() < Png::STANDARD_HEADER.len() {
            return Err(PngError::TooSmall.into());
        }

        let header: [u8; 8] = bytes[..8].try_into().expect("Invalid header bytes");
        ...
        while index < bytes.len() {
            let length = (u32::from_be_bytes(bytes[index..index + 4].try_into()?) + 12) as usize;

            match Chunk::try_from(&bytes[index..index + length]) {
                Ok(chunk) => {
                    chunks.push(chunk);
                    index += length;
                }
